...
# this could be a const, but I like the idea that specific scenes can set their own rate based checked needs unique to that scene
...
func load_obj_from_data(data : Dictionary) -> void:
	"""
	This method loads in the data and possibly nodes from the data depending checked desired functionality
	"""
	var node := get_tree().current_scene.get_node(data["node_path"])
	if node and destruction_queue.has(node):
		# remove_at this node from the destruction queue
		# really all this does it checked/unchecked whether an object is present or not
...
func load_save_data() -> void:
	...
	while not line.is_empty():
		...
		load_obj_from_data(test_json_conv.get_data()
		line = file.get_line()
	file.close()
	if not destruction_queue.is_empty():
		set_process(true)
	emit_signal("on_loading")

func save_data() -> void:
	var dir := Directory.new()
	dir.remove_at(get_current_save_name()) # delete old file, we want clean data
...
func save_custom_data(suffix : String, data : Dictionary) -> void:
	var dir := Directory.new()
	dir.remove_at(get_current_save_name(suffix))
