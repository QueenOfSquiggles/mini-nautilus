...
func _ready() -> void:
	...
	get_tree().create_timer(0.1).connect("timeout",Callable(self,"set").bind("accept_inputs", true))
...
func _input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("ui_cancel"):
		get_tree().create_timer(0.1).connect("timeout",Callable(self,"_on_BtnReturn_pressed"))
