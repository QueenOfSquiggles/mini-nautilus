...
func _ready() -> void:
	...
	if queued_settings.is_empty():
		queued_settings = default_settings.duplicate(true)
		save_settings()
	propogate_changes()
	connect("tree_exiting",Callable(self,"save_settings"))
	SaveData.connect("on_saving",Callable(self,"save_settings"))
	create_menu_items()
...
func create_env_setting_dropdown(key : String) -> Control:
	...
	label.text = key
	split.alignment = HBoxContainer.ALIGNMENT_CENTER
	...
	dropmenu.connect("item_selected",Callable(self,"on_option_selected").bind(key))
...
func try_set_current_environment() -> bool:
	var env :WorldEnvironment = get_tree().current_scene.find_child("WorldEnvironment") as WorldEnvironment
	if not env:
