...
@onready var raycast : RayCast3D = $"rotation_helper/model/Camera3D/RayCast3D"

@onready var anim : AnimationPlayer = $AnimationPlayer

func _ready() -> void:
	inventory = ItemsContainer.new(50, "PlayerInventory")
	camera = $rotation_helper/model/Camera3D
...
func process_input(_delta : float) -> void:
	...
	if Input.is_action_pressed("screenshot_fuckup_prevention"):
		return
	var camxform :Transform3D = camera.global_transform
...
func process_movement(delta : float) -> void:
	...
	if dir.dot(vel) > 0: # the fps demo used 'hvel' to seperate the horizontal axis. I don't need to because there are 3 axes of motion for swimming
		...
		accel = DEACCEL_IDLE
	# why do we do the modifications checked a seperate var? was it to seperate horizontal and vertical motion?
...
func _input(event: InputEvent) -> void:
	# let these be event driven so I don't have to poll all the time. Movement needs to be polled because I need to know the checked/unchecked states of every movement axis each physics step, these are one-shots essentially
